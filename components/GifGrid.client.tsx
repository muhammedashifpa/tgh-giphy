  ...
  return (
    <div className="space-y-12">
      <div className="columns-1 sm:columns-2 md:columns-3 lg:columns-4 gap-4 px-4">
        {gifs.map((gif, index) => (
          <div key={`${gif.id}-${index}`} className="relative break-inside-avoid mb-4 group rounded-xl overflow-hidden cursor-pointer bg-zinc-900 shadow-sm hover:shadow-xl transition-shadow duration-300">
            <Image
              src={gif.url}
              alt={gif.title}
              width={400}
              ...
              unoptimized // GIFs often benefit from being unoptimized to avoid quality loss in some proxies
            />
            {/* Overlay */}
            <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex flex-col justify-end p-4">
              <div className="flex items-center justify-between gap-2">
                ...
                <div className="flex gap-2">
                   <button className="p-2 bg-white/10 hover:bg-white/20 backdrop-blur-md rounded-full transition-colors">
      ...
      <div ref={loaderRef} className="py-12 flex justify-center">
        ...
        {!hasMore && gifs.length > 0 && (
          <p className="text-zinc-500 font-medium italic">You've reached the end of the galaxy.</p>
        )}
      </div>
    </div>
  );
